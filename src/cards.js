            ...
            `}>
                <List>
                    {Menucontent.map((item, index) => 
                        index < 7 ? <Listitem key={index} answer={item[3]} css={css`flex-basis: 25%;`} onClick={() => this.handleClick(index)}>{item[2]}{!this.state.isHidden && [<Modal content={this.state.content}  />,<Closer />]}</Listitem> : ''
                    )}
                </List>
            </div>
        )}
